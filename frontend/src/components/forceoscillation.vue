...
<div>
    ...
    text-align:left;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;">
        <div>振幅:{{controls.amplitude}}°</div>
        <div style="height:10px;"></div>
        <div>摆轮周期:{{T.toFixed(3)}}秒</div>
        <div style="height:10px;"></div>
        <div v-if="controls.vibration_type=='受迫振动'">
            <div>电机周期:{{(1/controls.powerv*60).toFixed(3)}}秒</div>
            <div style="height:10px;"></div>
            <div>摆轮电机相位差:{{deltaphi.toFixed(1)}}°</div> 
        </div>
    </div>

    <div style="z-index:1000;position:absolute;left:20px;top:200px;" v-show="showGraphs">
        <el-container>
            <el-main>
                <div id="echarts1" style="width:450px;height:300px;"></div>
                <div id="echarts2" style="width:450px;height:300px;"></div>
...
export default {
    data(){
        return{
            ...
            button1type:false,
            controls:{
                vibration_type:null,//振动类别
                // damping:false,
                // power:false,
                beta:0,
                theta_degree:0,//初始角度为0
                powerv:0,//电机转速 this.wf/2/Math.PI-0.1
                phi0:0,//初相位
                amplitude:'0',
            },//GUI界面
            floor:null,
            ...
            openflash:0,
            plate180graph_array:[0],
            powergraph_array:[0],
            plate180graph_array2:[0],
            powergraph_array2:[0],
            T10:0,
            }
    },
    methods:{
        ...
        init()
        {
            ...
            this.m=150/180*Math.PI*2*this.wf*0.253
        ...
        {    
            ...
            //阻尼振动
                ...
                if(this.t-this.count*this.T<0.1 && this.t-this.count*this.T>0){
                    this.controls.amplitude = parseInt(Math.abs(this.theta_end)*180/Math.PI)+'';
                ...
                if(this.last_powerv!=this.controls.powerv || this.t == 0){
                    this.plate180graph_array=[this.theta_end*180/Math.PI];
                    this.powergraph_array=[Math.sin(this.thetaright+Math.PI/2)*100];
                    this.last_powerv = this.controls.powerv 
                    this.t = 0 
                    //输入的电机频率是转速n *2pi变成omiga
                    this.powerw=2*Math.PI*this.controls.powerv/60   
                    //计算、调整相位差
                ...
                if(this.theta_end*this.theta_last<0 && this.group_trans_rotation < phi && this.group_trans_rotation + dt*this.powerw >= phi)
                {
                    ...
                    this.deltaphi = (this.group_trans_rotation)*180/Math.PI;
                ...
                if(this.plate180graph_array.length<600){
                ...
                this.plate180graph_array2.push(this.theta_end*180/Math.PI)
                this.powergraph_array2.push(Math.sin(this.thetaright+Math.PI/2)*100)
                if(this.plate180graph_array2.length>600){
                    this.plate180graph_array2.shift()
                    this.powergraph_array2.shift()
                }
                ...
                //绘图
                {
                    ...
                    this.echarts1.setOption(this.setEcharts(this.plate180graph_array,this.powergraph_array,'相位平衡阶段曲线'));
                    this.echarts2.setOption(this.setEcharts(this.plate180graph_array2,this.powergraph_array2,'电机摆轮相位曲线'))  
                }
            }
        ...
        addCamera()
        {
            ...
            this.camera.position.z=700;//40 40 40 //0 150 400
        ...
        addLight()
        {
            var spotlight1 = new THREE.SpotLight(0xffffff, 3,400,Math.PI/3)
            spotlight1.position.set(0,0,350)
            var spotlight2 = new THREE.SpotLight(0xffffff, 3,400,Math.PI/3)
        ...
        addFloor()
        {
            ...
            var boxTexture=[]
            boxTexture[3]=LoadTexture("../../static/images/woodtexture.jpg",8,8)
            for(var i=0;i<=5;i++){
                if(i==3) continue;
                boxTexture[i]=LoadTexture("../../static/images/hardwood2_diffuse.jpg",1,1)
            }
            var boxH = 5000
        ...
        stop(){
            ...
            if(this.controls.vibration_type=='自由振动'){
                ...
                //gui回到原样
                {
                    this.controls.powerv = 35
                    this.enabledGuiGroup3()
                }
                //清空图
                {
                    this.plate180graph_array=[0];
                    this.powergraph_array=[0];
                    this.plate180graph_array2=[0];
                    this.powergraph_array2=[0];
                }
                {
                    this.AWW0 = new Map();
                    this.PhiWW0 = new Map();
        ...
        addGui()
        {
            ...
            this.controls=
            {
                ...
                beta:0.053,
                theta_degree:0,//初始角度为0
                powerv:35,//电机频率 this.wf/2/Math.PI-0.1
                phi0:Math.PI/2,//初相位
                amplitude:'0',
                measure:()=>{
        ...
        addGui_group2(){
            ...
            this.beta = this.gui_group2.add(this.controls,"beta",{'阻尼1':0.093,'阻尼2':0.095,'阻尼3':0.097}).name('阻尼系数');
        ...
        addGui_group3(){
            ...
            this.gui_group3.add(this.controls,"powerv",30,45).name("电机转速").listen();
            this.beta = this.gui_group3.add(this.controls,"beta",{'阻尼1':0.253,'阻尼2':0.255,'阻尼3':0.257}).name('阻尼系数').listen();
